...
def residual_bottleneck_block(filters, stage, block, strides=None, attention=None, cut='pre'):
    ...
    def layer(input_tensor):
        ...
        if cut == 'pre':
            ...
            shortcut = layers.SeparableConv2D(filters * 4, (1, 1), name=sc_name, strides=strides, **conv_params)(x)
        else:
            raise ValueError('Cut type not in ["pre", "post"]')

        # continue with convolution layers
        x = layers.SeparableConv2D(filters, (1, 1), name=conv_name + '1', **conv_params)(x)

        x = layers.BatchNormalization(name=bn_name + '2', **bn_params)(x)
        x = layers.Activation('relu', name=relu_name + '2')(x)
        x = layers.ZeroPadding2D(padding=(1, 1))(x)
        x = layers.SeparableConv2D(filters, (3, 3), strides=strides, name=conv_name + '2', **conv_params)(x)

        x = layers.BatchNormalization(name=bn_name + '3', **bn_params)(x)
        x = layers.Activation('relu', name=relu_name + '3')(x)
        x = layers.SeparableConv2D(filters * 4, (1, 1), name=conv_name + '3', **conv_params)(x)
...
def ResNet(model_params, input_shape=None, input_tensor=None, include_top=True,
    ...
    x = layers.SeparableConv2D(init_filters, (7, 7), strides=(2, 2), name='conv0', **conv_params)(x)
